...
def getModelCNNWindow3(sequence, window_size=3, n_features=1,epochs=100):
    ...
    es = EarlyStopping(monitor='val_loss', min_delta=0.001, patience=0, verbose=0, mode='auto', baseline=None, restore_best_weights=False)
    model.fit(X, y, epochs=epochs, callbacks=[es])

    return model


def getModelCNNWindow24(sequence, window_size=int(24), activation='relu', n_features=1, epochs=200 ):
